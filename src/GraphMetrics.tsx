...
const getStrategicResourceSample = (graph: MultiDirectedGraph): string[] => {
  if (!isValidGraph(graph)) return [];
  const resourceNodes = graph.filterNodes((node) => graph.getNodeAttributes(node).type === "resource");
  if (resourceNodes.length <= 40) return resourceNodes;
  ...
  const scoredResources = resourceNodes.map((node) => {
    ...
    const score = degree * 2 + outDegree * 1.5 + inDegree * 1.0 + (attrs.quantity || 0) * 0.1;
    return { node, score, outDegree, inDegree };
  });
  scoredResources.sort((a, b) => b.score - a.score);

  const categories: Record<string, string[]> = {
    high_demand: [], high_supply: [], balanced: [], producer_only: [], consumer_only: [],
  };
...
const findAllPathsBetweenResources = (graph: MultiDirectedGraph): Record<string, Record<string, string[][]>> => {
  if (!isValidGraph(graph)) return {};
  ...
  for (const start of sampledResources) {
    ...
    for (const end of sampledResources) {
      ...
      const paths = findAllPathsDFS(graph, start, end, 10, 10);
      if (paths.length > 0) {
...
const calculateProductionConsumption = (graph: MultiDirectedGraph): ProductionConsumptionStats => {
  ...
  const producedResources = resources.filter((r) => (r.production ?? 0) > 0).sort((a, b) => (b.production ?? 0) - (a.production ?? 0));
  const consumedResources = resources.filter((r) => (r.consumption ?? 0) > 0).sort((a, b) => (b.consumption ?? 0) - (a.consumption ?? 0));
  return {
    producedResources,
    consumedResources,
...
const calculateNodeTypeStats = (graph: MultiDirectedGraph) => {
  const stats = { resource: { count: 0, totalQuantity: 0 }, card: { count: 0 } };
...
const calculateEdgeTypeStats = (graph: MultiDirectedGraph) => {
  const stats = { gain: { count: 0, totalQuantity: 0 }, cost: { count: 0, totalQuantity: 0 } };
  if (!isValidGraph(graph)) return stats;
...
const findShortestPaths = (allPaths: Record<string, Record<string, string[][]>>): Record<string, Record<string, string[]>> => {
  ...
  for (const [start, ends] of Object.entries(allPaths)) {
    ...
    for (const [end, paths] of Object.entries(ends)) {
      if (paths.length > 0) {
        shortestPaths[start][end] = paths.reduce((shortest, current) => (current.length < shortest.length ? current : shortest));
      }
    }
  }
...
const BalanceAnalysisPanel: React.FC<{ metrics: GraphMetricsData }> = ({ metrics }) => {
  ...
  const generateProblems = (): Problem[] => {
    ...
    const globalRatio = metrics.productionConsumption.totalProduction / (metrics.productionConsumption.totalConsumption || 1);
    if (globalRatio > 1.5) {
      problems.push({
          ...
          <>
            ...
            >
              <Box sx={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 2 }}>
                <MetricItem label="Nós" value={metrics.basic.order} tooltip="Total de recursos e cartas no jogo" />
                <MetricItem label="Arestas" value={metrics.basic.size} tooltip="Número total de relações entre cartas e recursos" />
                <MetricItem
                  label="Densidade"
                  value={metrics.basic.density}
                  tooltip="Proporção de conexões existentes vs possíveis. Valores altos indicam sistema complexo."
                />
                <MetricItem label="Direcionado" value="Sim" tooltip="As relações têm direção definida (produção/consumo)" />
                <MetricItem label="Multigrafo" value="Sim" tooltip="Permite múltiplas conexões entre os mesmos nós" />
              </Box>
            </MetricSection>
                ...
                <>
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    {metrics.pathAnalysis.cycleExamples.map((cycle, i) => (
                      ...
                      >
                        <Typography variant="body2" fontFamily="monospace" sx={{ color: "var(--accent-color)" }}>
                          {cycle.join(" → ")} → {cycle[0]}
                        </Typography>
                      </Paper>
                    ))}
                ...
                <Box sx={{ maxHeight: 500, overflow: "auto", pr: 1 }}>
                  {Object.entries(metrics.pathAnalysis.shortestPaths).map(([start, ends]) => (
                    ...
                    >
                      <Typography
                        variant="subtitle1"
                        sx={{ color: "var(--accent-color)", fontWeight: "bold", mb: 1, borderBottom: "1px solid var(--accent-color)", pb: 0.5 }}
                      >
                        De: {start}
                      </Typography>
                      <Box sx={{ ml: 2 }}>
                          ...
                          .map(([end, path]) => (
                            <Box key={end} sx={{ mb: 2 }}>
                              ...
                              <Typography
                                ...
                                sx={{ ml: 2, color: "#4CAF50", wordBreak: "break-all" }}
                              >
                                {path.join(" → ")}
                              </Typography>
